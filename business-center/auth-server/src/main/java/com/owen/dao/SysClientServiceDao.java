...
public interface SysClientServiceDao {
    ...
    @Insert("insert into t_oauth_client_service(clientId, serviceId) values(#{clientId}, #{serviceId})")
    ...
    @Select("select t.serviceId from t_oauth_client_service t where t.clientId = #{clientId}")
